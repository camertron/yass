...
pub struct YAnimationDuration {
    values: CachedValueList<AnimationDuration<Time>>
}
...
pub struct YAnimationDurationValue {
    ...
    time: RefCell<Option<Opaque<typed_data::Obj<YTime>>>>
}

impl YAnimationDurationValue {
    pub fn new(animation_duration: AnimationDuration<Time>) -> Self {
        Self { animation_duration, time: RefCell::new(None) }
    }
    ...
    pub fn time(ruby: &Ruby, rb_self: typed_data::Obj<Self>) -> Option<Value> {
        match rb_self.animation_duration {
            AnimationDuration::Time(time) => {
                if rb_self.time.borrow().is_none() {
                    *rb_self.time.borrow_mut() = Some(Opaque::from(ruby.obj_wrap(YTime::new(time))));
                }

                Some(ruby.get_inner(rb_self.time.borrow().unwrap()).as_value())
            }
...
impl DataTypeFunctions for YAnimationDurationValue {
    fn mark(&self, marker: &gc::Marker) {
        if let Some(time) = self.time.borrow().as_ref() {
            marker.mark(*time);
        }
    }
}
